...
node_editor(struct nk_context *ctx)
{
    ...
    if (nk_begin(ctx, "NodeEdit", nk_rect(0, 0, 1920,1080),NK_WINDOW_NO_SCROLLBAR))
    {
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodedit->node_count);
        {
            ...
            while (it) {
                ...
                /* execute node window */
                if (nk_group_begin(ctx, it->name, NK_WINDOW_MOVABLE|NK_WINDOW_NO_SCROLLBAR|NK_WINDOW_TITLE))/**/
                {
...
//                    it->color.a = (nk_byte)nk_propertyi(ctx, "#A:", 0, it->color.a, 255, 1,1);
    			...
    			/* node connector and linking */
                    ...
                    if (nk_input_has_mouse_click_down_in_rect(in, NK_BUTTON_LEFT, nk_rect(it->bounds.x - nodedit->scrolling.x, it->bounds.y - nodedit->scrolling.y, it->bounds.w, it->bounds.h), nk_true))
                    {
//                        bounds = nk_layout_space_rect_to_local(ctx, node->bounds);
...
//                        bounds.y += nodedit->scrolling.y;
//                        it->bounds = bounds;
                        nk_stroke_rect(canvas,nk_rect(it->bounds.x - nodedit->scrolling.x, it->bounds.y - nodedit->scrolling.y,it->bounds.w + 8, it->bounds.h ), 10, 2, nk_rgba(250, 250, 0,250));
                    }

                    nk_fill_rect(canvas,nk_rect(it->bounds.x - nodedit->scrolling.x, it->bounds.y - nodedit->scrolling.y,it->bounds.w + 8, it->bounds.h ), 10, nk_rgba(35, 35, 35,150));
                    for (n = 0; n < it->output_count; ++n) {
                        struct nk_rect circle;
                        circle.x = node->bounds.x + node->bounds.w-4;
                        circle.y = node->bounds.y + space * (float)(n+1);
                        circle.w = 8; circle.h = 8;
                        nk_stroke_circle(canvas, circle, 1 ,nk_rgb(100, 100, 100));
                    ...
                    for (n = 0; n < it->input_count; ++n) {
                        ...
                        circle.y = node->bounds.y + space * (float)(n+1);
                        circle.w = 8; circle.h = 8;
                        nk_stroke_circle(canvas, circle, 1, nk_rgba(100, 100, 100,200));
        ...
        if (nk_input_is_mouse_hovering_rect(in, nk_window_get_bounds(ctx)) && nk_input_is_mouse_down(in, NK_BUTTON_MIDDLE)) {
